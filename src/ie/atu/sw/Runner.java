...
public class Runner {
	...
	private void query() {
		...
		out.println("\n[Query 10] Show all Customers with a status of RECURRING");
		root.stream().filter(c -> c.getStatus() == CustomerStatus.RECURRING).forEach(out::println);

		// Query 11: Show all Orders with a value of more than 1000
		out.println("\n[Query 11] Show all Orders with a value of more than 1000");
		root.stream().flatMap(c -> c.orders().stream()).filter(o -> o.total() > 1000).forEach(out::println);

		// Query 12: Show the total value of all Orders for each Customer
		out.println("\n[Query 12] Show all Order Items that start with an Order Number of QB-10");
		root.stream().flatMap(c -> c.orders().stream()).flatMap(o -> Arrays.stream(o.items()))
				.filter(i -> i.number().startsWith("QB-10")).forEach(out::println);
